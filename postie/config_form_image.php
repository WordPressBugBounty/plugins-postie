...
<div id="simpleTabs-content-4" class="simpleTabs-content">
    <table class='form-table'>
        ...
        <?php
        echo PostieAdmin::boolean_select_html(__("Use First Image as Featured Image", 'postie'), "postie-settings[featured_image]", $featured_image, __("If any images are attached, the first one will be the featured image for the post", 'postie')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        echo PostieAdmin::boolean_select_html(__("Include Featured Image in Post", 'postie'), "postie-settings[include_featured_image]", $include_featured_image, __("Should the featured image be included in the post.", 'postie')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        echo PostieAdmin::boolean_select_html(__("Automatically insert image gallery", 'postie'), "postie-settings[auto_gallery]", $auto_gallery, __("If any images are attached, they will automatically be inserted as a gallery. If the 'Preferred Text Type' is 'HTML' this will add a galery in addition to the images in the email.", 'postie')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        echo PostieAdmin::select_html(__("Gallery Link Type", 'postie'), "postie-settings[auto_gallery_link]", $auto_gallery_link, array(__('Default', 'postie'), __('Post', 'postie'), __('File', 'postie'), __('None', 'postie')), "Select the type of link the gallery should use"); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        echo PostieAdmin::boolean_select_html(__("Image/Attachment Location", 'postie'), "postie-settings[images_append]", $images_append, __("Location of attachments. Before or After content. For 'html' content this will only affect attachments that are not inline.", 'postie'), array(__('After', 'postie'), __('Before', 'postie'))); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        echo PostieAdmin::boolean_select_html(__("Start Image Count At", 'postie'), "postie-settings[start_image_count_at_zero]", $start_image_count_at_zero, __('For use if using "Image Place Holder Tag" below.', 'postie'), array(__('Start at 0', 'postie'), __('Start at 1', 'postie'))); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        echo PostieAdmin::boolean_select_html(__("Use custom image field for images", 'postie'), "postie-settings[custom_image_field]", $custom_image_field, __("When set to 'Yes' no images will appear in the post (other attachment types will be processed normally). Instead the url to the attachment will be put into a custom field named 'image'. Your theme will need logic to display these attachments", 'postie')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        ...
        ?>
        <tr> 
            <th scope="row"><?php esc_html_e('Image Place Holder Tag', 'postie') ?></th> 
            <td>
                ...
                <p class='description'><?php esc_html_e("For use in 'plain' messages. The code for inserting an image. I.e. put \"#img1# in your email where you want the first image to show. See also \"Start Image Count At\"", 'postie') ?></p>
            </td> 
        </tr> 
        <tr>
            <th scope="row"><?php esc_html_e('Image Template', 'postie') ?></th>
            <td>
                ...
                <p class='description'><?php esc_html_e('Note that this template are only used if the "Preferred Text Type" setting is set to "plain"', 'postie'); ?></p>
                <p class='description'><?php echo wp_kses_post(__('Sizes for thumbnail, medium, and large images can be chosen in the <a href="options-media.php">Media Settings</a>. The samples here use the default sizes, and will not reflect the sizes you have chosen.', 'postie')); ?></p>
                <div style="margin-top: 10px; font-weight: bold;"><?php esc_html_e('Preview', 'postie'); ?></div>
